...
var browser = UE.browser = function(){
    ...
    /**
	 * 浏览器版本判断 IE系列返回值为5,6,7,8,9,10等 gecko系列会返回10900，158900等. webkit系列会返回其build号
	 * (如 522等).
	 * 
	 * @name version
	 * @grammar UE.browser.version => number
	 * @example if ( UE.browser.ie && UE.browser.version == 6 ){ alert(
	 *          "Ouch!居然是万恶的IE6!" ); }
	 */
	 ...
	 * @example if ( UE.browser.isCompatible ){ alert( "你的浏览器相当不错哦！" ); }
	 */
...
var utils = UE.utils = {
	 ...
	 *       iterator的上下文
	 * @example UE.utils.each([1,2],function(v,i){ console.log(v)//值
	 *          console.log(i)//索引 })
	 ...
	 *          UE.utils.inherits(SubClass,SuperClass); var sub = new
	 ...
	 * @grammar UE.utils.defer(fn,delay,exclusion) =>fn
	 *          //延迟delay毫秒执行fn，若exclusion为真，则互斥执行fn
	 * @example function test(){ console.log("延迟输出！"); } //非互斥延迟执行 var testDefer =
	 *          UE.utils.defer(test,1000); testDefer(); => "延迟输出！"; testDefer(); =>
	 *          "延迟输出！"; //互斥延迟执行 var testDefer1 =
	 *          UE.utils.defer(test,1000,true); testDefer1(); => //本次不执行
	 *          testDefer1(); => "延迟输出！";
	 */
	 ...
	 * 
	 * @name listToMap
	 * @grammar UE.utils.listToMap(list) => Object
	 ...
	 *          UE.utils.unhtml(html); ==> &lt;body&gt;You say:&quot;你好！Baidu
	 *          &amp; UEditor!&quot;&lt;/body&gt; UE.utils.unhtml(html,/[<>]/g)
	 ...
	 * @example UE.utils.isEmptyObject({}) ==>true UE.utils.isEmptyObject([])
	 *          ==>true UE.utils.isEmptyObject("") ==>true
	 */
...
var domUtils = dom.domUtils = {
	 ...
	 * @grammar UE.dom.domUtils.findParent(node,filterFn) => Element
	 *          //filterFn为过滤函数，node作为参数，返回true时才会将node作为符合要求的节点返回
	 * @grammar UE.dom.domUtils.findParent(node,filterFn,includeSelf) => Element
	 *          //includeSelf指定是否包含自身
	 */
	 ...
	 * @grammar UE.dom.domUtils.findParentByTagName(node,tagNames) => Element
	 *          //tagNames支持数组，区分大小写
	 ...
	 * @grammar UE.dom.domUtils.findParents(node,includeSelf,filterFn) => Array
	 ...
	 * @example <b></b><i></i>xxxx<b>bb</b> --> xxxx<b>bb</b>
	 */
	 ...
	 * 
	 * @name split
	 * @grammar UE.dom.domUtils.split(node,offset) => TextNode
	 *          //返回从切分位置开始的后一个文本节点
	 */
	 ...
	 * @example UE.dom.domUtils.on(document.body,"click",function(e){
	 *          //e为事件对象，this为被点击元素对戏那个 })
	 * @example UE.dom.domUtils.on(document.body,["click","mousedown"],function(evt){
	 ...
	 * @example <span style="font-size:12px">ssss</span> and <span
	 *          style="font-size:12px">bbbbb</span> => true <span
	 *          style="font-size:13px">ssss</span> and <span
	 *          style="font-size:12px">bbbbb</span> => false
	 */
	 ...
	 * @desc <code type="html"><b>ooo</b>是node节点
	 ...
	 * @example
	 *       <p>
	 *       <span style="font-size:12px;">xx<span style="font-size:12px;">aa</span>xx</span>
	 *       </p>
	 ...
	 * @grammar UE.dom.domUtils.getElementsByTagName(node,tagName) => Array
	 *          //节点集合数组
	 */
	 ...
	 * @example <span style="color:#fff"><span style="font-size:12px">xxx</span></span>
	 *          ==> <span style="color:#fff;font-size:12px">xxx</span>
	 */
	 ...
	 *          //ignoreNext指定是否忽略右兄弟
	 * @example <b>xxxx</b><b>ooo</b><b>xxxx</b> ==> <b>xxxxoooxxxx</b>
	 */
	 ...
	 *          UE.dom.domUtils.removeAttributes(node,["id","name"]); //After
	 ...
	 * @grammar UE.dom.domUtils.getComputedStyle(element,styleName) => String
	 *          //返回对应样式名称的样式值
	 ...
	 * @example //执行方法前的dom结构 <span class="test1 test2 test3">xxx</span> //执行方法
	 *          UE.dom.domUtils.removeClasses(element,["test1","test3"])
	 *          //执行方法后的dom结构 <span class="test2">xxx</span>
	 */
    ...
    /**
	 * 在元素element上增加一个样式类className，支持以空格分开的多个类名 如果相同的类名将不会添加
	 * 
	 * @name addClass
	 ...
	 * @example UE.dom.domUtils.filterNodeList(document.getElementsByTagName('*'),'div
	 *          p') //返回第一个是div或者p的节点
	 *          UE.dom.domUtils.filterNodeList(document.getElementsByTagName('*'),function(n){return
	 *          n.getAttribute('src')}) //返回第一个带src属性的节点
	 *          UE.dom.domUtils.filterNodeList(document.getElementsByTagName('*'),'i',true)
	 *          //返回数组，里边都是i节点
	 */
...
(function () {
    ...
    Range.prototype = {
		 ...
		 * @example DOM Element : <b>x<i>x[x<i>xx]x</b> //执行方法后 返回的fragment里的
		 *          dom结构是 <i>x<i>xx dom树上的结构是 <b>x<i>x<i>|x</b> 注意range改变了
		 ...
		 * 
		 * @name getCommonAncestor
		 * @grammar range.getCommonAncestor([includeSelf, ignoreTextNode]) =>
		 *          Element
		 * @example <b>xx[xx<i>xx]x</i>xxx</b> ==>getCommonAncestor() ==> b
		 *          <b>[<img/>]</b> range.startContainer ==> b
		 *          range.startOffset ==> 0 range.endContainer ==> b
		 *          range.endOffset ==> 1 range.getCommonAncestor() ==> b
		 *          range.getCommonAncestor(true) ==> img <b>xxx|xx</b>
		 *          range.startContainer ==> textNode range.startOffset ==> 3
		 *          range.endContainer ==> textNode range.endOffset ==> 3
		 ...
		 * @grammar range.applyInlineStyle(tagName) => Range
		 *          //tagName为需要添加的样式标签名
		 * @grammar range.applyInlineStyle(tagName,attrs) => Range
		 *          //attrs为属性json对象
		 * @desc <code type="html"><p>xxxx[xxxx]x</p>  ==>  range.applyInlineStyle("strong")  ==>  <p>xxxx[<strong>xxxx</strong>]x</p>
		 ...
		 * @grammar range.removeInlineStyle(tagNames) => Range //tagNames
		 *          为需要去掉的样式标签名,支持"b"或者["b","i","u"]
		 * @desc <code type="html">xx[x<span>xxx<em>yyy</em>zz]z</span>  => range.removeInlineStyle(["em"])  => xx[x<span>xxxyyyzz]z</span></code>
		 */
...
//
		 ...
		 * 
		 * @name scrollToView
		 * @grammar range.scrollToView([win,offset]) => Range
		 *          //针对window对象，若不指定，将以编辑区域的窗口为准,offset偏移量
		 */
...
(function () {
    ...
    Editor.prototype = {
		 ...
		 * @example var editor = new UE.ui.Editor(); editor.render("myEditor");
...
// });
		 ...
		 * @example var editor = new UE.ui.Editor() editor.ready(function(){
...
// //ie ff不能从外边拖入
// //chrome只针对从外边拖入的内容过滤
// var innerDrag = 0, source = browser.ie ? me.body : me.document,
// dragoverHandler;
...
// }
		 ...
		 * @example var dialog = editor.getDialog("insertimage"); dialog.open();
		 *          //打开dialog dialog.close(); //关闭dialog
		 */
    };
...
UE.ajax = function() {
    ...
    return {
		 ...
		 * @example UE.ajax.request('http://www.xxxx.com/test.php',{
		 *          //可省略，默认POST method:'POST', //可以自定义参数 content:'这里是提交的内容',
		 *          //也可以直接传json，但是只能命名为data，否则当做一般字符串处理 data:{ name:'UEditor',
		 *          age:'1' } onsuccess:function(xhr){
		 *          console.log(xhr.responseText); }, onerror:function(xhr){
...
// img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' +
// (first.border||0) +"px solid #000";
 ...
 * @name baidu.editor.execCommand
 * @param {String}
 *            cmdName directionality执行函数的参数
 * @param {String}
 *            forward ltr从左向右输入，rtl从右向左输入
 */
(function() {
...
UE.plugins['undo'] = function () {
    ...
    function UndoManager() {
        ...
        this.getScene = function () {
            ...
            // trace:3461
            // 这个会引起回退时导致空格丢失的情况
// browser.ie && (cont = cont.replace(/>&nbsp;</g, '><').replace(/\s*</g,
// '<').replace(/>\s*/g, '>'));
...
(function () {
	 ...
	 * @return { align: 'left' || 'right' || 'center', valign: 'top' || 'middle' ||
	 *         'bottom' } 或者 null
	 */
    ...
    function showError(e) {
		 ...
		 * Created with JetBrains PhpStorm. User: taoqili Date: 13-2-20 Time:
		 * 下午6:25 To change this template use File | Settings | File Templates.
		 */
;
(function () {
    var UT = UE.UETable,
